...
- 本游戏并不是长期作战游戏，所以如果你想在重启Koishi实例后仍然继续上一次游戏，你可以使用以下命令：
- 通过`导出游戏状态`来导出一个游戏状态的JSON
- `导入游戏状态 <JSON>`来导入游戏目前所处的状态，这样就可以跨实例保存数据了
- 如果你能理解JSON的键值对信息，你也可以进行自定义对局，例如玩家开局拥有无限金钱，进行乱斗，或者让玩家的血量设定为一个很高的数值，亦或者修改当前步数……
